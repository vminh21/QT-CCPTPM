...
/**
 * HomePage - Bảng điều khiển (Dashboard) chính của hội viên.
 */
function HomePage() {
  ...
  // Lấy danh sách tin tức và huấn luyện viên nổi bật (giới hạn 4 bản ghi mỗi loại)
  useEffect(() => {
    Promise.all([
  ...
  // Hợp phần hiển thị đánh giá số sao (Star Rating)
  const Stars = ({ n }) => (
    <span style={{ display: 'flex', alignItems: 'center' }}>
      {[1, 2, 3, 4, 5].map(i => (
        <i 
          key={i} 
          className={i <= Math.round(n) ? 'bx bxs-star' : 'bx bx-star'} 
          style={{ color: i <= Math.round(n) ? '#f59e0b' : '#3f3f46', fontSize: '0.9rem' }}
        ></i>
      ))}
    </span>
  );
  ...
  return (
    <MemberLayout title="Tổng quan Hội viên">
      {/* Các liên kết nhanh dạng Thẻ thống kê */}
